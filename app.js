...
/* --- NEW: CONFIGURATION FOR EXPORT FIXES --- */
const MEGA_STONE_EXCEPTIONS = {
    ...
    'Mewtwo X': 'Mewtwonite X', 'Mewtwo Y': 'Mewtwonite Y', 'Ampharos': 'Ampharosite', 'Aggron': 'Aggronite'
};
    ...
    for (let mon of monsToFetch) {
        ...
        if(baseName.includes("Mega")) {
            ...
            if(MEGA_STONE_EXCEPTIONS[lookupName]) {
                ...
                // 2. Default Rule: Name + "ite" (e.g. Pidgeot -> Pidgeotite)
                megaStone = cleanBase + "ite" + (suffix ? " " + suffix : "");
            }
            
            // Special Orbs logic (unchanged)
            ...
            // Use base name for set lookup
            baseName = cleanBase;
        }

        // --- CHECK PREDEFINED COMPETITIVE SETS FIRST ---
        if (COMPETITIVE_SETS[baseName]) {
            set = COMPETITIVE_SETS[baseName];
        } 
        // --- IF NOT PREDEFINED, LOOKUP IN SETDEX ---
        ...
        if (set) {
            // --- BUILD EXPORT ---
            // Determine Item: Mega must have Stone, otherwise use Set Item, otherwise Boots/Leftovers
            let item = isMega && megaStone ? megaStone : (set.item || 'Heavy-Duty Boots');
            ...
            try {
                ...
                // Use Mega Stone if applicable, otherwise Heavy-Duty Boots (better fallback than leftovers)
                exportText += `${baseName} @ ${isMega && megaStone ? megaStone : 'Heavy-Duty Boots'}\n`;
